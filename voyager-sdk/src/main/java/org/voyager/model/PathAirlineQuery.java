...
public class PathAirlineQuery {
    ...
    public static class PathAirlineQueryBuilder {
        ...
        public PathAirlineQuery build() {
            PathAirlineQuery pathAirlineQuery = new PathAirlineQuery(originIATAList,destinationIATAList,
                    airline,excludeIATAList,excludeRouteIdList,excludeFlightNumberList,limit);
            JakartaValidationUtil.validate(pathAirlineQuery);
            pathAirlineQuery.originIATAList = originIATAList.stream().map(String::toUpperCase).toList();
            pathAirlineQuery.destinationIATAList = destinationIATAList.stream()
                    .map(String::toUpperCase).toList();
            if (excludeIATAList != null) {
                pathAirlineQuery.excludeIATAList = excludeIATAList.stream().map(String::toUpperCase).toList();
            }
            if (excludeFlightNumberList != null) {
                pathAirlineQuery.excludeFlightNumberList = excludeFlightNumberList.stream()
                        .map(String::toUpperCase).toList();
            }
            return pathAirlineQuery;
        }
    }
}

...
import org.voyager.config.external.FlightRadarConfig;
...
import static org.voyager.utils.HttpRequestUtils.fetchDocumentFromURLWithRetry;
...
public class FlightRadarService {
    ...
    public static Either<ServiceError, List<AirportFR>> extractAirportRoutes(String iata) {
        String requestURL = String.format(flightRadarConfig.getAirportRoutesPathWithParam(),iata);
        Either<ServiceError, Document> either = fetchDocumentFromURL(requestURL);
        if (either.isLeft()) return Either.left(either.getLeft());
        Document document = either.get();
        return extractAirportsFromDocument(document,requestURL);
    }

    public static Either<ServiceError, Airline> extractAirlineFromFlightData(String flightNumber) {
        Either<ServiceError, Document> either = fetchDocumentFromURLWithRetry(
                String.format(flightRadarConfig.getFlightPathWithParam(),flightNumber));
        if (either.isLeft()) return Either.left(either.getLeft());
        Document document = either.get();
        return extractAirlineFromDocument(document);
    }
        ...
        try {
            ...
            if (response.statusCode() == 429) {
                ...
                while (attempt < maxRetries) {
                    ...
                    LOGGER.info(String.format("will attempt %d/%d retires after %dms (%dsec) to %s",
    ...
    public static Either<ServiceError, Option<AirportScheduleFR>> extractAirportResponseWithRetry(String iata, Airline airline) {
        String requestURL = String.format(flightRadarConfig.getAirportPathWithParams(),airline.getPathVariableFR(),iata);
        try {
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(new URI(requestURL))
                    .headers(flightRadarConfig.getAuthHeader(), flightRadarConfig.getAuthValue())
            ...
            return processResponse(response,iata,airline,requestURL);
        } catch (IOException | InterruptedException | URISyntaxException e) {
            return Either.left(new ServiceError(HttpStatus.INTERNAL_SERVER_ERROR,
                    e.getMessage(), e));
        }
    ...
    private static Either<ServiceError, Option<AirportScheduleFR>> processResponse(HttpResponse<String> response, String iata, Airline airline, String requestURL) {
        if (response.statusCode() == 429) {
        ...
        return convertResponseBody(response.body(), iata, airline,requestURL);
    }
    ...
    private static Either<ServiceError, Airline> extractAirlineFromDocument(Document document) {
        Element metaDescription = document.select("meta[name=description]").first();
        if (metaDescription == null) return Either.left(new ServiceError(HttpStatus.NOT_FOUND,new ServiceException("Flight document missing airline description element")));
        String content = metaDescription.attr("content");
        Pattern pattern = java.util.regex.Pattern.compile("\\((.*?)\\)");
        Matcher matcher = pattern.matcher(content);

        if (matcher.find()) {
            String airlineString = matcher.group(1);
            try {
                return Either.right(Airline.fromDisplayText(airlineString));
            } catch (IllegalArgumentException e) {
                return Either.left(new ServiceError(HttpStatus.NOT_FOUND,new ServiceException(
                        String.format("Airline string %s from document does not match any existing airlines",
                                airlineString))));
            }
        } else {
            return Either.left(new ServiceError(HttpStatus.NOT_FOUND,new ServiceException("Flight document missing airline description element")));
        }
    }

    private static Either<ServiceError,List<AirportFR>> extractAirportsFromDocument(Document document,String requestURL) {
        ...
        if (element != null) {
            ...
            try {
                ...
                return convertRoutesObjectToAirportsJson(arrRoutes);
    ...
    private static Either<ServiceError,List<AirportFR>> convertRoutesObjectToAirportsJson(Object arrRoutes) {
        ObjectMapper om = new ObjectMapper();
        try {
            String arrRoutesJsonString = om.writeValueAsString(arrRoutes);
            return extractAirports(arrRoutesJsonString);
        } catch (JsonProcessingException e) {
            return Either.left(new ServiceError(HttpStatus.INTERNAL_SERVER_ERROR,
                    String.format("Error while writing object as json string. Message: %s. Object: %s",
                            e.getMessage(), arrRoutes.toString()), e));
    ...
    private static Either<ServiceError,List<AirportFR>> extractAirports(String arrRoutesJsonString) {
        LOGGER.trace(arrRoutesJsonString);
        ObjectMapper om = new ObjectMapper();
        try {
            List<AirportFR> airportFRList = om.readValue(arrRoutesJsonString, new TypeReference<List<AirportFR>>(){});
            return Either.right(airportFRList);
        } catch (JsonProcessingException e) {
            return Either.left(new ServiceError(HttpStatus.INTERNAL_SERVER_ERROR,
                    e.getMessage(),e));
        }
    ...
    private static Either<ServiceError, Option<AirportScheduleFR>> convertResponseBody(String jsonBody, String iata, Airline airline, String requestURL) {
        if (StringUtils.isBlank(jsonBody) || jsonBody.equals("[]")) {
            ...
            try {
                ...
                LOGGER.error(String.format(
                        "JsonProcessingException after successful fetch of AirportScheduleFR at %s, error: %s",
    ...
    private static Either<ServiceError, Option<AirportScheduleFR>> convertAirportResponseBody(String jsonBody, String airportCode1, String airportCode2, String requestURL) {
        if (StringUtils.isBlank(jsonBody) || jsonBody.equals("[]")) {
            LOGGER.info(String.format("%s airport returns no flights for airport %s",
    ...
    private static Either<ServiceError, Option<AirportDetailsFR>> convertAirportDetailsResponseBody(String jsonBody,
                                                                                            ...
                                                                                            String requestURL) {
        if (StringUtils.isBlank(jsonBody) || jsonBody.equals("[]")) {
            LOGGER.info(String.format("%s airport returns no details",iata));
            return Either.right(Option.none());
        } else {
            try {
                ...
                LOGGER.error(String.format(
                        "JsonProcessingException after successful fetch of AirportDetailsFR at %s, error: %s",
                        requestURL,e.getMessage()));
                return Either.left(new ServiceError(HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage(), e));
            }
        }
